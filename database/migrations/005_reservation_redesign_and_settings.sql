...
--       A reservation may be for a walk-in with no user account,
--       so it can identify the buyer either by linking an existing
--       lead OR by capturing a manual buyer name + phone.
...
--     * Configurable hold period, DEFAULT 15 DAYS; auto-release if
--       not marked Sold in time.
...
COMMENT ON COLUMN dealer_settings.reservation_hold_days IS
  'Configurable reservation hold period in days (default 15). If a reservation is not marked Sold within this window it is auto-released back to Live.';
...
-- buyer. The buyer may be an existing user (lead) or a walk-in captured
-- manually, so user_id is relaxed to NULL and manual buyer fields are added.
...
-- A reservation must identify its buyer by a linked user/lead OR a manual name+phone.
ALTER TABLE reservations
  ADD CONSTRAINT chk_reservation_buyer_identified CHECK (
    user_id IS NOT NULL
    OR lead_id IS NOT NULL
    OR (buyer_name IS NOT NULL AND buyer_phone IS NOT NULL)
  );

COMMENT ON COLUMN reservations.buyer_name IS
  'Walk-in buyer name when the reservation is not linked to a registered user/lead.';
...
COMMENT ON COLUMN reservations.token_amount_paise IS
  'Optional offline token amount (paise) for staff reference/display only. No money movement is performed by the platform.';
