...
- Reservation service (**Hub Admin-only**): `POST /v1/admin/reservations` (buyer via linked lead OR manual name+phone; `token_received`+optional `token_amount_paise`, display-only), optimistic lock on `row_version`, `uq_reservations_active_car`, **configurable hold (default 15 days) auto-release job**, state machine (`Reserved → DealInProgress → Sold`/`Released`/`Cancelled`), **reserved-car lock** (reject interest/TD/second reservation), **Reserved Vehicles** worklist with `daysPending`, `notify-employee` action.
