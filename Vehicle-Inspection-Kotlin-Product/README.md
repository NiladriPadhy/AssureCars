...
- **Organisation identity** — the signed-in user's **Organisation Name** is shown as the company
  name on the report screen and PDF cover.
...
are filled in, the app runs in **offline baseline mode** (bundled questionnaire) — everything still
works, just without RTDB sync.

The report/PDF company name comes from the signed-in user's **Organisation Name** (returned by the
login/subscription Cloud Functions); no separate branding configuration is required.
